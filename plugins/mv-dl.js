...
cmd({
  ...
  if (!q) {
    return await conn.sendMessage(from, { text: "🔍 Usage: *.movie <movie name>*" }, { quoted: mek });
  }
...
Reply with a *number* to choose the platform 👇

1. BAISCOPES Search
2. CINESUBZ Search
3. SINHALASUB Search
4. SINHALASUBS Search
5. SUBLK Search
6. PIRATE Search
7. PUPILVIDEO Search
8. MOVIEPRO Search
...
`;
  ...
  global.numReply = global.numReply || {};
  global.numReply[from] = {
    type: "movie_search",
    query: q,
    time: Date.now()
  };
...
event.on("message", async (conn, mek, m) => {
  try {
    const from = mek.key.remoteJid;
    const body = m.text?.trim();

    if (!global.numReply || !global.numReply[from]) return;

    const data = global.numReply[from];
    if (data.type !== "movie_search") return;
    ...
    switch (num) {
      case 1: cmd = `.baiscope ${q}`; break;
      case 2: cmd = `.cinesubz ${q}`; break;
      case 3: cmd = `.sinhalasub ${q}`; break;
      case 4: cmd = `.sinhalasubs ${q}`; break;
      case 5: cmd = `.sublk ${q}`; break;
      case 6: cmd = `.pirate ${q}`; break;
      case 7: cmd = `.pupilvideo ${q}`; break;
      case 8: cmd = `.moviepro ${q}`; break;
      default:
        return await conn.sendMessage(from, { text: "❌ Invalid option. Use *1–8*." });
    }

    delete global.numReply[from]; // clear state

    await conn.sendMessage(from, { text: `🔎 Searching...\n\n${cmd}` });
    return conn.sendMessage(from, { text: cmd });
